...
const GO_BACK = html`<div class="d-flex flex-column justify-content-between align-items-center"> 
...
export default class MfaDevices extends LitElement {
  static get properties() {
    return {
      ...
      deviceName: { type: String, attribute: false },
      useWebAuthnDevice: { type: Boolean, attribute: false },
      encodedSecret: { type: String, attribute: false },
      generatedQrCode: { type: String, attribute: false },
      totpCode: { type: String, attribute: false }
    };
  }
  ...
  static finalizeStyles() {
    return [
      ...
      ::-webkit-input-placeholder { opacity: 1; -webkit-transition: opacity 0.3s; transition: opacity 0.3s; }  /* Chrome <=56, Safari < 10 */
      :-moz-placeholder { opacity: 1; -moz-transition: opacity 0.3s; transition: opacity 0.3s; } /* FF 4-18 */
      ::-moz-placeholder { opacity: 1; -moz-transition: opacity 0.3s; transition: opacity 0.3s; } /* FF 19-51 */
      :-ms-input-placeholder { opacity: 1; -ms-transition: opacity 0.3s; transition: opacity 0.3s; } /* IE 10+ */
      ::placeholder { opacity: 1; transition: opacity 0.3s; } /* Modern Browsers */
    ...
    <form class="d-flex flex-column h-100" @submit="${e => { e.preventDefault(); this.error = null; this.registerDevice(); }}">
      <div class="mx-3 my-3">
        ...
        <div>
          ...
          <fieldset class="mt-1">
            <div class="mt-2 d-sm-flex justify-content-around">
              <div>
                <input type="radio" name="deviceType" id="deviceType-webauthn" ?checked=${this.useWebAuthnDevice} @change="${() => this.useWebAuthnDevice = true}">
                <label for="deviceType-webauthn">Hardware Device</label>
              </div>
              <div>
                <input type="radio" name="deviceType" id="deviceType-authenticator" ?checked=${!this.useWebAuthnDevice} @change="${() => { this.useWebAuthnDevice = false; this.registerDevice(); }}">
                <label for="deviceType-authenticator">Mobile Authenticator</label>
              </div>
            </div>
          </fieldset>
        </div>
    ...
    <form class="d-flex flex-column" class="max-height: 500px;" @submit="${e => { e.preventDefault(); this.error = null; this.state = states.NEW; this.requestUpdate(); }}">
      ...
      <div class="d-flex justify-content-center">
        <div>
          ...
          || html`
          <button type="submit" class="create-button btn btn-sm mt-3">
  ...
  handleTotpCode(rawCode) {
    let replacementCode = rawCode;

    replacementCode = replacementCode.replace(/[^\d]+/gi, '');
    if (replacementCode.length > 6) {
      replacementCode = replacementCode.substring(0, 6);
    }

    const totpCodeInputElement = this.shadowRoot.getElementById('totpCode');
    if (totpCodeInputElement.value !== replacementCode) {
  ...
  displayQrCodeForNewAuthenticator() {
    const generateQrCode = async () => {
      ...
      this.encodedSecret = [...secret].map(val => base32hexMap[val % 32]).join('');

      const totpSecretUri = `otpauth://totp/${encodeURIComponent(deviceString)}?secret=${this.encodedSecret}&issuer=${encodeURIComponent(window.location.hostname)}`;
      QRCode.toDataURL(totpSecretUri).then(result => {
        this.generatedQrCode = result;
      }).catch(error => {
        // eslint-disable-next-line no-console
        console.error('QR Code Generator Failed', error);
      });
    };
    ...
    <form class="d-flex flex-column h-100" @submit="${e => { e.preventDefault(); this.error = null; this.completeAuthenticatorRegistration(); }}">
      <div class="mx-md-3 my-md-3">
        ...
        <div>
          <fieldset>
            <label class="input-label">1. Scan this QR Code:</label>
            <div class="mt-2 d-flex justify-content-center">
              <img src="${this.generatedQrCode}">
            </div>
          </fieldset>
  ...
  getRender() {
    ...
    if (this.state === states.NEW_AUTHENTICATOR) {
      return this.displayQrCodeForNewAuthenticator();
    }
